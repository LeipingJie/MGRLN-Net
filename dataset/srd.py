...
class SRD_Dataset(LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int,
    ...
    def prepare_data(self):
        self.train_root = os.path.join(self.data_dir, 'training')
        self.val_root = os.path.join(self.data_dir, 'testing')
        
    def setup(self, stage=None):
        # Assign train/val datasets for use in dataloaders
        if stage == 'fit' or stage is None:
            self.train_data = ImageFolders('train', self.train_root, self.scale_size, self.crop_size,
                                           ['shadow', 'mask', 'shadow_free'], 
                                           ['shadow', 'mask', 'free'], 
