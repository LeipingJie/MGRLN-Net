...
class AISTD_Dataset(LightningDataModule):
    def __init__(
        self,
        data_dir: str,
        batch_size: int,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_data = ImageFolders('train', self.train_root, self.scale_size, self.crop_size,
                                           ['train_A', 'train_B', 'train_C_fixed_ours'], 
                                           ['shadow', 'mask', 'free'], 
        ...
        if stage == 'val' or stage is None:
            self.val_data = ImageFolders('val', self.val_root, None, None, 
                                           ['test_A', 'test_B', 'test_C_fixed_official'], 
                                           ['shadow', 'mask', 'free'], 
                                           [self.t_shadow, self.t_mask, self.t_mask], 
                                           self.return_name)
...
if __name__ == '__main__':
    ...
    dst = AISTD_Dataset(r'../../AISTD_Dataset', 1, 0, True, (448, 448), (400, 400))
    dst.setup()
